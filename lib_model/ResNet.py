...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("pool out.shape:",out.shape)
        out = out.view(out.size(0), -1)
        #print("view out.shape:",out.shape) #imagenet 128,1024
        out = self.linear(out)
        return out

class ResNetFeature(nn.Module):
    def __init__(self,  block=BasicBlock, num_blocks=[2, 2, 2, 2], num_classes=10, imagenet = False):
        super(ResNetFeature, self).__init__()
        self.name = 'ResNetFeature'
        self.in_planes = 64
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
...
class ResNetLinear(nn.Module):
    def __init__(self, block=BasicBlock, num_blocks=[2, 2, 2, 2], num_classes=10, imagenet = False):
        ...
        self.name = 'ResNetLinear'
        if(imagenet):
            self.linear = nn.Linear(512 * 4, 100)
        else:
...
def ResNet18(num_classes=10,  imagenet = False):
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, imagenet = imagenet)
